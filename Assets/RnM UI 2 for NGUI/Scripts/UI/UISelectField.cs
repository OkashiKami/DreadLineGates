...
public class UISelectField : UIWidgetContainer
{
	...
	void Start()
	{
		...
		{
			this.fieldNormalSprite = this.backgroundElement.spriteName;
			this.fieldNormalColor = this.backgroundElement.color;
		}
		
		if (this.labelElement != null)
			this.labelNormalColor = this.labelElement.color;
		
		if (this.arrowElement != null)
			this.arrowClosedSprite = this.arrowElement.spriteName;
		...
		for (int i = 0, imax = this.items.Count; i < imax; ++i)
		{
			...
			if (i != (imax - 1) && !string.IsNullOrEmpty(this.listSeparatorSprite))
			{
				...
				if (separator)
				{
					...
					separator.transform.localPosition = new Vector3(this.listSepMarginLeft, (offsetY - this.listSepMarginTop), 0f);
					separator.Update();
	...
	{
		UISpriteData sData = this.atlas.GetSprite(this.listSeparatorSprite);
