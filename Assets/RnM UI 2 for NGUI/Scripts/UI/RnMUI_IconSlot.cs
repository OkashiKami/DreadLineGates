...
public class RnMUI_IconSlot : MonoBehaviour
{
	...
	protected virtual void OnDrag(Vector2 delta)
	{
		...
		this.OnDragDropMove(delta * (this.mRoot == null ? 1f : this.mRoot.pixelSizeAdjustment));
	}
	...
	protected virtual void OnDragDropRelease(GameObject surface)
	{
		...
		{
			// No target slot
			// Check if we handle custom surface
			if (!this.HandleCustomSurface(surface))
			{
				// Try to throw away the slot content
				this.OnThrowAway();
			}
	...
	/// <param name="surface">Surface.</param>
	protected virtual bool HandleCustomSurface(GameObject surface)
	{
