...
[ExecuteInEditMode, DisallowMultipleComponent, AddComponentMenu("RPG and MMO UI/Window")]
public class UIWindow : MonoBehaviour {
	...
	protected virtual void Awake()
	{
		...
		foreach (UIWindow window in windows)
		{
			...
			{
				Debug.LogWarning("UIWindow: A window with ID (" + this.WindowId + ") already exists, using next available.");
	...
	protected virtual void Start()
	{
		...
		if (this.contentHolder == null) this.contentHolder = this.transform;
	...
	public void HookFocusEvents(bool hook)
	{
		...
		foreach (UIWidget widget in widgets)
		{
			...
			{
				UIEventListener el = UIEventListener.Get(widget.gameObject);
				
				if (hook)
				{
					el.onPress += OnWidgetPress;
				}
				else
				{
					el.onPress -= OnWidgetPress;
